...
var title, Description, year, category, price, model, image, uid, adId;
var post = document.getElementById("posted");
function like(thisRef) {
    console.log("I AM IN FAVORATE");
    console.log("AdId => ", thisRef.id); //Get Current Post ID
    console.log("My UID =>", auth.currentUser.uid); //Get Current User ID
    db.collection("ads")
        .onSnapshot((snapshot) => {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            db.collection("favorite").doc(user.uid).collection("ads").doc(adId).set({
                title: title,
                ...
                console.log("Added Favorite DB");
                alert("Favorite is Added");
            }).catch(() => {
                console.log("Error in Favorite DB");
            })
        }
    });
}
...
function getFav() {
    var posts = document.getElementById("posted");
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            db.collection("favorite").doc(user.uid).collection("ads")
                .onSnapshot((snapshot) => {
                    post.innerHTML = "";
                    snapshot.forEach((doc) => {
                        ...
                        if ("email" in localStorage) {
                            ...
                            btnFav.setAttribute("type", "button");
                            btnFav.setAttribute("class", "btn btn-danger");
                            btnFav.setAttribute("onclick", "unlike(this)");
                            btnFav.innerHTML = "<i class='glyphicon glyphicon-heart'></i> UnMark as Favorite";
                            btnFav.setAttribute("id", doc.id);
                            var idFav = btnFav.getAttribute("id");
...
function unlike(thisRef) {
    ...
    firebase.auth().onAuthStateChanged(function (user) {
        if (user) {
            db.collection("favorite").doc(user.uid).collection("ads").doc(adId).delete()
                .then(() => {
